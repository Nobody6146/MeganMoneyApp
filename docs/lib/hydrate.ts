...
class HydrateDOMOptions
{
    ...
    modifiers: HydrateHTMLAttributeModifiers;
...
class HydrateModelEvent {
    ...
    constructor(type:HydrateModelEventType, model: any, previousState: any, propName: string, target: HTMLElement, hydrate: HydrateApp, modelName?: string)
    {
...
class HydrateApp
{
    ...
    parent(search: string | object) : any {
    ...
    async dispatch(type: HydrateModelEventType,  model: any, propName: string, previousState: any, target: HTMLElement, mode:HydrateDispatchMode, modelName?:string): Promise<void> {
        ...
        let listeningElements:HTMLElement[] = [];
        if(target.isConnected)
        {
            if(target.matches(selector))
                listeningElements.push(target);
            listeningElements = listeningElements.concat(<HTMLElement[]>[...target.querySelectorAll(selector)]);
        }
    ...
    #loadDomAttributes(target : HTMLElement): HydrateDomUpdateAttributes {
        const app = this;
        let getArguments = function(target: HTMLElement, name: string): HydrateDomAttributeArguments[] {
    ...
    #createLocalizedEvent(target: HTMLElement, event:HydrateModelEvent, propName: string) {
        let type = event.type;
    ...
    #getScriptValue(expression:string, event:HydrateModelEvent, prop, route: HydrateRouteRequest) {
    ...
    async #insertComponent(template: Node[], event: HydrateModelEvent, propNames:string[], append:boolean, routeRequest:HydrateRouteRequest) {
        ...
        const insertModelName = function(element:HTMLElement, propName: string) {
        ...
        const initializeComponent = function(element:HTMLElement, propName: string) {
        ...
        propNames.forEach(propName => {
            template.forEach(x => {
                ...
                node.querySelectorAll<HTMLElement>(modelSelector).forEach(element => {
                ...
                node.querySelectorAll<HTMLElement>(initializeSelector).forEach(element => {
